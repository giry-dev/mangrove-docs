...
When creating a Direct contract, the first thing you need is to import the necessary contracts needed for the constructor. Next is creating a constructor that calls the constructor of Direct. We set the gas requirement to 30.000, since we are not doing much in the posthook<!-- FIXME: link to explanation of gas requirement and posthook -->, it does not need anymore than that.
...
Since we now can post new offers, these offer will get taken at some point. And if one of the offers was taken, we wanted to retract the other offers, since we no longer have the funds for that offer. To do this we use the hook `posthookSuccess`. This hook gets called if the offer was successfully taken.

The first thing we want to do is use Directs own implementation of `posthookSuccess`. This makes sure to repost the offer if it was only partially taken. Next we need to figure out which of the two offers was taken. In the `SingleOrder`we can get what the inbound token was for the taken offer. Using this we can figure out if it was `STABLE1` or `STABLE2`, and thereby also the offer id.

Next we need to know whether the Directs `posthookSuccess` reposted the offer or not. Posthooks returns values that the user can use, to know what the hook did and if the was a success. In this case, if the posthook return `posthook/reposted` it means that the posthook successfully reposted the offer. Knowing that the offers was reposted, we now know that the other offer also needs to be updated, to use the correct gives and wants.

To do this we need information about the remaining gives so we use `residualGives`, which calculated the remaining gives. And we again use `MGV.offers()` and `MGV.offerDetails` to get information about the other offer. With this information we now know what the old wants was for the offer, the old and new gives for the offer. Using this we can calculate what the new wants should be. <!-- FIXME: explain the unchecked -->

With this we can now use Directs own `updateOffer` (you should always use the contracts own newOffer, updateOffer or retractOffer, since it might do some extra bookkeeping). We use the same gas requirement as the old offer, since it still requires the same amount of gas. We use a pivot id right next to the old, since this offer basically the same price as the old one. If we use zero as pivot id, it might be very gas costly the find the right position for the offer.<!-- FIXME: Link to description of pivot ids --> When the offer is updated, we return a value that indicates that this hook successfully reposted both offers.
