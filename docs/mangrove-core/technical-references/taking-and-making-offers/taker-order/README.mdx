...
Every Mangrove[ offer list ](../market.md#general-structure)can be either [active or inactive](../../governance-parameters/local-variables.md#de-activating-an-offer-list), and Mangrove itself can be either [alive or dead](../../governance-parameters/global-variables.mdx#other-governance-controlled-setters). Taking offers is only possible when Mangrove is alive on offer lists that are active.
...
2. Mangrove sends _inbound_ tokens to the current offer's associated [logic](../reactive-offer/maker-contract.mdx).
...
Any failed [offer](../reactive-offer/) execution results in a [bounty](../reactive-offer/offer-provision.mdx#computing-the-provision-and-offer-bounty) being sent to the caller as compensation for the wasted gas.
...
* `fee` is the amount of `outbound_tkn` that was sent to Mangrove's vault in payment of the potential %%fee|fee%% associated to the `(outbound_tkn, inbound_tkn)`[offer list](../market.md#general-structure).&#x20;
...
It is also possible to target specific offer IDs in the [offer list](../market.md). This is called **Offer Sniping**.
