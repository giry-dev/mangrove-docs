...
We are now ready to buy some DAI using a FoK order. It is very simple to do. Using the same method as for a market order (`buy`), we just give it an extra parameter `mangroveOrder`. In this parameter we set `fillOrKill` to true. This way we use the [MangroveOrder](../../strat-lib/technical-references/code/strategies/MangroveOrder) contract to buy, were as, had we left out the `mangroveOrder` parameter, we would have used the Mangrove protocol directly and created a market order.
